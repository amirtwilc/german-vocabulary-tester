...
const supportedPresentPeople = new Set<PresentPerson>([
  'ich',
  'du',
  'erSieEs',
  'ihr',
...
const questionFacet = (id: string) => {
  const person = id.match(
    /-(present|preterite)-(ich|du|erSieEs|wir|ihr|sieSie)$/,
  );
  ...
  return (
    [
      ...
      'comparative',
      'superlative',
    ].find((facet) => id.endsWith(`-${facet}`)) ?? id
  );
};
...
const translationOptions = (
  correct: string,
  pool: readonly { english: string }[],
  random: () => number,
) => {
  const distractors = [
    ...new Set(
      pool.map((item) => item.english).filter((answer) => answer !== correct),
    ),
  ];
  return shuffle(
...
const nounBlock = (
...
): RawQuizQuestion[] => [
  {
    ...
    wordType: 'noun',
    word: noun.german,
    eyebrow: 'Noun · meaning',
    prompt: `What does “${noun.german}” mean?`,
    mode: 'choice',
    correctAnswer: noun.english,
    options: translationOptions(noun.english, pool, random),
  },
  {
    id: `${noun.id}-article`,
    wordId: noun.id,
...
const verbCandidates = (
  ...
  for (const [person, answer] of Object.entries(verb.present ?? {}) as [
    PresentPerson,
    string,
  ][]) {
    if (!supportedPresentPeople.has(person)) continue;
    candidates.push({
      id: `${verb.id}-present-${person}`,
      wordId: verb.id,
      wordType: 'verb',
  ...
  for (const [person, answer] of Object.entries(verb.preterite ?? {}) as [
    PresentPerson,
    string,
  ][]) {
    candidates.push({
      id: `${verb.id}-preterite-${person}`,
      wordId: verb.id,
      wordType: 'verb',
      word: verb.infinitive,
  ...
  return shuffle(
    candidates.filter(
      (question) =>
        !excludedQuestionKeys.has(keyQuestion(question).questionKey),
    ),
    random,
  ).slice(0, 3);
};

const adjectiveAdverbBlock = (
  item: AdjectiveAdverb,
  pool: readonly AdjectiveAdverb[],
  random: () => number,
): RawQuizQuestion[] => {
  ...
  return [
    {
      ...
      wordType: item.kind,
      word: item.german,
      eyebrow: `${label} · meaning`,
      prompt: `What does “${item.german}” mean?`,
      mode: 'choice',
      correctAnswer: item.english,
      options: translationOptions(item.english, pool, random),
    },
    ...(item.comparative
      ? [
          {
            id: `${item.id}-comparative`,
            wordId: item.id,
            wordType: item.kind,
            word: item.german,
...
const prepositionBlock = (
  ...
  const base = {
    ...
    word: preposition.german,
    mode: 'choice' as const,
    options: shuffle(['Akkusativ', 'Dativ'], random),
  };
  if (preposition.usage === 'fixed') {
    return [
      {
        ...base,
        id: `${preposition.id}-case`,
        eyebrow: 'Preposition · grammatical case',
        prompt: `Which case does “${preposition.german}” take?`,
        correctAnswer: preposition.case,
      },
    ];
  }
  return [
    {
      ...base,
      id: `${preposition.id}-movement`,
      eyebrow: 'Two-way preposition · movement',
      prompt: `Which case does “${preposition.german}” take with movement toward a destination?`,
      correctAnswer: 'Akkusativ',
    },
    {
      ...base,
      id: `${preposition.id}-location`,
      eyebrow: 'Two-way preposition · no movement',
      prompt: `Which case does “${preposition.german}” take for a fixed location?`,
      correctAnswer: 'Dativ',
      options: shuffle(['Akkusativ', 'Dativ'], random),
    },
  ];
...
export const createQuiz = (
  ...
  const blocks = shuffle(
    [
      ...
      ...source.adjectivesAndAdverbs.map((item) =>
        adjectiveAdverbBlock(item, source.adjectivesAndAdverbs, random),
      ),
    ],
    random,
  );
  const questions = blocks
    .flat()
    .map(keyQuestion)
    .filter((question) => !excludedQuestionKeys.has(question.questionKey));
  return questions.slice(
    0,
    Math.max(0, Math.min(Math.floor(amount), questions.length)),
  );
};

export const getMaximumQuestionCount = (
  source: Vocabulary,
  excludedQuestionKeys: ReadonlySet<string> = new Set(),
) =>
  createQuiz(source, Number.MAX_SAFE_INTEGER, () => 0.5, excludedQuestionKeys)
    .length;
...
export const normalizeAnswer = (answer: string) =>
  answer
    ...
    .replace(/ü/g, 'ue')
    .replace(/ß/g, 'ss');
export const isCorrectAnswer = (answer: string, correct: string) =>
